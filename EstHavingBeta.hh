#ifndef EST_HAVING_BETA_HH
#define EST_HAVING_BETA_HH
 ...
 *  group by A
 *  having sum(B) in [.,.]a
 *  using the method a beta-distribution
 */
...
class EstHavingBeta {
  public:
    ...
    using dist_t       = gm::dist_beta_t;
  public:
    EstHavingBeta();
    EstHavingBeta(const uint64_t     aCardR,
  ...
  public:
    ...
    inline const dist_param_t& dist_param_Cnt() const { return _attr_stat_Cnt.dist_param(); }
    inline double min_Cnt()  const { return attr_stat_Cnt().min(); }
    inline double max_Cnt()  const { return attr_stat_Cnt().max(); }
    inline double mean_Cnt() const { return attr_stat_Cnt().mean(); }
    inline double var_Cnt()  const { return attr_stat_Cnt().variance(); }
    inline double skew_Cnt() const { return attr_stat_Cnt().skew(); }
